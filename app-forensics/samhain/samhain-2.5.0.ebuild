...
SRC_URI="http://www.la-samhna.de/archive/samhain_signed-${PV}.tar.gz"
LICENSE="GPL-2"
SLOT="0"
IUSE="crypt debug doc login-watch mounts-check mysql netclient netserver postgres prelude static suidcheck userfiles xml"
...
#INSTALL_NAME=""
#
# SENDER can be set to the e-mail address or user Samhain should use
# as the From address when sending e-mails.
#SENDER=""
#
# MAGIC_WORD can be set to disable command line parsing unless the magic word
# is the first argument on the command line.  When used, additional arguments
# must be passed via stdin. (ex: "echo -t init -p notice | samhain magicword")
#MAGIC_WORD=""
...
src_compile() {
	...
	if [[ -n "${SENDER}" ]] ; then
		myconf="${myconf} --with-sender=${SENDER}"
	fi

	if [[ -n "${MAGIC_WORD}" ]] ; then
		myconf="${myconf} --enable-nocl=${MAGIC_WORD}"
	fi
...
src_install() {
	...
	insopts -m0600
	if [[ -n "${INSTALL_NAME}" ]] ; then
		newinitd init/samhain.startGentoo "${INSTALL_NAME}"
		dosed "s:samhain:${INSTALL_NAME}:g" "/etc/init.d/${INSTALL_NAME}"
	else
...
pkg_postinst() {
	if [[ -n "${STEALTH}${INSTALL_NAME}" ]] ; then
		if use doc ; then
			ewarn
			ewarn "Be aware that Samhain's man pages and documentation WERE installed with the"
			ewarn "application.  To further obscure Samhain's presence, you should emerge with"
			ewarn "USE=\"-doc\"."
		else
			elog
			elog "Manual pages and documentation were NOT installed in order to obscure"
			elog "Samhain's presence. You should also remove Samhain's installation traces"
			elog "from ${ROOT}var/cache/edb/world and ${ROOT}var/db/pkg."
		fi
	else
		elog
